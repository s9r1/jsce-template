...
%\documentclass[noflushend]{jjsce}% Proof
%\documentclass[english]{jjsce}% Proof
\usepackage{amsmath}
...
\makeatother
\begin{Eabstract}
The Japan Society of Civil Engineers (JSCE) provides 
a (u)p\LaTeXe\ class file, named \ClassFile, for its journal. 
This document describes the usage of the class file.
\end{Eabstract}
\fi
\begin{keyword}
...
（以下，英文タイトル部と略します）を
表示するために利用します（\ref{sec:flushend}節，\pageref{sec:flushend}頁参照）．
\end{itemize}
...
（\ref{sec:flushend}節，\pageref{sec:flushend}頁参照）．
\item 
著者名は，以下のように記述します．
\begin{verbatim*}
\authorlist{%
...
節が一つの場合は，\verb/\section*/ とすることもできます．
\item 
...
和文投稿原稿の場合，英文タイトル・著者名・英文要旨
（以下，「英文タイトル部」と略します）は，
最終ページで本文を並行止め（左段と右段を揃える）した後に，
英文タイトル部を出力するスペースがある場合はその下部に出力され，
スペースがない場合は次のページに出力されます
（\texttt{zref} および \texttt{flushend} パッケージを利用しており，
後者はクラスファイル内部で読み込みます）．

\texttt{flushend} パッケージは左段と右段を揃えようとしますが，
最終ページに図表や見出しが多い場合に左段が右段よりかなり高くなる
体裁になったり，最終ページが数行程度の場合に左右段が並行にならないといった
現象が生じることがあります（バグと限界により多くの場合は解決が困難です）．

このような場合の解決方法として以下の4つを試してください．
\begin{itemize}
\item
左段の任意の箇所で \verb/\newpage/ を記述して改段を試みる．
ただし，\verb/\newpage/ で改段できない（つまり記述しても無効）ことがあります
（これは \texttt{flushend} パッケージの仕様です）．

\item
英文タイトル部を出力するスペースがページ下部にある場合，
または英文タイトル部が本文と重なったり接近しすぎるようなとき，
\begin{verbatim}
\aboveEtitlesep10mm
\end{verbatim}
と記述すると（\texttt{10mm}は仮），英文タイトル部が下がります．

\item
英文タイトル部を出力するスペースがページ下部にない場合，
左段と右段の高低差分（例えば\texttt{40mm}であれば）を
\begin{verbatim}
\enlargeRcolheight40mm
\end{verbatim}
などと記述してください．これは右段に架空のスペースを加えることを意味します．

\item
\ref{sec:note}節で説明するいくつかの方法で，
ページ全体の容量を減らすなどの対策をとる．
\end{itemize}

しかし，上のような方法でも解決できない場合は，
以下のような方法があります（標準の \verb/\twocolumn/ の挙動に戻します）．
\begin{itemize}
\item
まず，ドキュメントクラスのオプションに \texttt{noflushend} を指定します．
\begin{verbatim}
...
英文タイトル部がページ下部に収まる場合，
最終ページのテキストの左段の部分で，
以下のようにして英文タイトル部を出力します．
必要に応じて，\verb/\makeEtitle/ の上下を \verb/\vspace/ を使って調節します．
\begin{verbatim}
\begin{figure}[b]
\vspace{-10mm}
\makeEtitle
